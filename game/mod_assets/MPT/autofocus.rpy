...
init 2 python:
    def autofocus(char, event, interact=True,*args,**kwargs):
        ...
        global af_enabled
        if not interact or not renpy.showing(char) or not renpy.get_at_list(char) or renpy.get_at_list(char)[0] in exception_transforms or not af_enabled: #Bail if the speaking character doesn't logically make sense to apply logic to.
            return
